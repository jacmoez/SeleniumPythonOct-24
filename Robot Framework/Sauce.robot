#pip3 install robotframework
#pip3 robotframework-seleniumlibirary

*** Settings ***
Library                     SeleniumLibrary
Suite Setup                 Open Browser             ${url}             ${browser}

*** Variables ***
${url}                     https://www.saucedemo.com/
${browser}                  Chrome


*** Test Cases ***
Login With Valid
        Input Text              id=user-name        standard_user
        Sleep                   2s
        Input Text              name=password       secret_sauce
        Sleep                   2s
        Press Keys              id=user-name        ENTER
        Sleep                   2s
        [Documentation]         Test 1: Login Success

Check Inventory Page
        Page Should Contain            Products
        [Documentation]                Test 2: Product Page Success

Add To Cart
       Click Button          id=add-to-cart-sauce-labs-backpack
       Sleep                 2s
       Click Button          name:add-to-cart-sauce-labs-fleece-jacket
       Sleep                 2s
       Click Button          xpath=//*[@id="add-to-cart-test.allthethings()-t-shirt-(red)"]
       Sleep                2s

Remove Item From Cart
       Click Button             id=remove-sauce-labs-backpack
       Sleep                    2s

View Cart
       Click Element            class=shopping_cart_link
       Sleep                    2s
       Page Should Contain      Your Cart
       Click Element            xpath=/html/body/div/div/div/div[2]/div/div[1]/div[4]/div[2]/a/div
       Sleep                   2s
       Click Button             id=remove
       Sleep                   2s
       Click Element            class=shopping_cart_link
       Sleep                    2s

Checkout
      Click Button              id=checkout
      Sleep                     2s
      Input Text                id=first-name                                        QA
      Sleep                     2s
      Input Text                name=lastName                                         Testing
      Sleep                     2s
      Input Text                css=input[placeholder="Zip/Postal Code"]                1111
      Sleep                     2s
      Click Button              id=continue
      Sleep                     2s

Verify Payment
      ${item_total}=            Get Text               class=summary_subtotal_label
      ${tax_total}=             Get Text               class=summary_tax_label
      ${total_price}=           Get Text               class=summary_total_label
      Log                       ${item_total}
      Log                       ${tax_total}
      Log                       ${total_price}

Finish
      Click Button              css=.btn.btn_action.btn_medium.cart_button
      Page Should Contain       Thank you for your order!
      Sleep                     5s
      Click Button              id=back-to-products
      Sleep                     5s